...
# DONE DOCUMENT IMMUTABILITY RULE

You MUST NEVER modify markdown files whose frontmatter contains:
...
These files are considered immutable finalized knowledge artifacts.

---

# DONE FILE PROTECTION RULES
...
* NEVER append autogenerated content
* NEVER restructure the file
* NEVER rename the file
* NEVER move the file
* NEVER delete the file
* NEVER update links automatically inside the file
* NEVER modify frontmatter values
* NEVER change formatting
* NEVER inject additional sections

These files are LOCKED.

---

# ALLOWED OPERATIONS FOR DONE FILES
...
The content itself MUST remain untouched.

---

# REGENERATION BEHAVIOR

If analysis detects that a target markdown document already exists and contains:
...
1. mark the document as immutable
2. skip regeneration entirely
3. preserve hashes/state as stable
4. remove the file from regeneration queues
5. avoid future write attempts

Log the skip operation inside:
...
```markdown
- skipped immutable done document: diagrams/payment-flow.md
```
...
MUST automatically receive classification:
...
These files MUST be treated as finalized artifacts.

---

# QUEUE EXCLUSION RULE

Done documents MUST NEVER appear in:

* pending regeneration queues
* pending work queues
* unstable documentation reports
* rewrite batches
* cleanup batches
...
The `status: Done` frontmatter has HIGHER PRIORITY than:

* regeneration logic
* hash mismatches
* architecture drift detection
* formatting normalization
* automatic semantic restructuring
* link optimization
* documentation enrichment

Even if the system believes improvements are possible, the file MUST remain untouched.

---

# IMMUTABLE FRONTMATTER DETECTION

Frontmatter detection MUST be whitespace tolerant.

The following MUST all be treated as immutable:
...
# MANUAL OWNERSHIP ASSUMPTION

Files marked as Done are assumed to be:

* manually validated
* manually curated
* architect-approved
* finalized knowledge artifacts

The system MUST defer completely to human ownership.
...
### NEW FILE FRONTMATTER DEFAULT RULE

All newly created markdown files inside the system (including `.memory/` outputs and any repository-derived artifacts) MUST include a standardized YAML frontmatter block.

This rule is mandatory for all *new file generation* and applies before any content is written.
...
* MUST be derived from:
  ...
  * filename, OR
  * primary concept of the document, OR
  * analyzed module/component name
* MUST be updated dynamically based on actual content meaning (not literal filenames when misleading)

---

### 2. tags

* MUST be context-aware and reflect:

  * domain (e.g. billing, identity, infrastructure)
  * technical role (e.g. api, service, handler, repository)
  * architecture patterns (e.g. cqrs, mediatr, ddd)
* For visualization/diagram-related files, ALWAYS include:

  * `diagram`
  * `visualization`
  * `mermaid` (when applicable)
...
* MUST default to:
  ...
  status: Draft
  ```
* MUST NOT be inferred from content unless explicitly specified
* MUST remain unchanged unless an explicit state transition system is invoked
...
* MUST default to:
  ...
  license: GPLv3
  ```
* MUST be inherited consistently across all generated files unless a repository-level override is explicitly defined
* MUST NOT be modified ad-hoc per file

---

## INTEGRATION RULE WITH EXISTING SYSTEM

* This rule applies ONLY to newly created files
* It MUST NOT override or modify existing files
* It MUST NOT affect any file marked:

  ```yaml
  status: Done
  ```
* It MUST be applied before writing file content to disk or memory structure

---

## CONSISTENCY REQUIREMENT

All generated files MUST maintain uniform frontmatter structure to ensure:

* deterministic indexing
* Obsidian compatibility
* stable memory graph generation
* predictable AI retrieval behavior
