...
# ./logmx.command logs/my_log.txt
#    -> Will open file "logs/my_log.txt" in LogMX
# ./logmx.command logs/my_log.txt logs/my_log2.txt
#    -> Will open files "logs/my_log.txt" and "logs/my_log2.txt" in LogMX
# ./logmx.command --conf=/mypath/my_startup.conf logs/my_log.txt
...
# Remove macOS quarantine flag to prevent AppTranslocation and other "security" issues
xattr -dr com.apple.quarantine "$LOGMX_PATH"
if [ "$1" == "--fix-macos" ]; then
	echo "Done."
	exit 0
fi
