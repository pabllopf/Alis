...
namespace Alis.Aspect.Debug
{
    ...
    public static class Logger
    {
        ...
        {
            if (Level == LogLevel.Info) Write(CreateText(LogType.Trace, ""), ConsoleColor.DarkGray);
        }
        ...
        {
            if (Level == LogLevel.Info) Write(CreateText(LogType.Trace, message), ConsoleColor.DarkGray);
        }
        ...
        {
            if (Level == LogLevel.Info || Level == LogLevel.Log)
        ...
        {
            if (Level == LogLevel.Info || Level == LogLevel.Log || Level == LogLevel.Normal)
        ...
        {
            if (Level == LogLevel.Info || Level == LogLevel.Log || Level == LogLevel.Normal || Level == LogLevel.Critical)
        ...
        private static string CreateText(LogType level, string message)
        {
            ...
            var parameters = stack.GetFrame(2)?.GetMethod()?.GetParameters();
            var parametersText = "";
        ...
        private static void Write(string text, ConsoleColor color)
        {
            ...
            File.WriteAllText(LogFile.Name, JsonConvert.SerializeObject(Logs, Formatting.Indented));
            Console.ResetColor();
        }
