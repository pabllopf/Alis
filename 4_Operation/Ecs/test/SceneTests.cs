    ...
    public class SceneTests
            {
                ...
                {
                    using Scene world1 = new();
        
                    ushort id;
                    using (Scene world2 = new())
                    {
                ...
                public void Query_IncludesComponents()
                {
                    ...
                    var query = scene.Query<With<int>, With<Class1>, With<Struct1>>();
            ...
            scene.TagDetached += (e, t) => Assert.True(true);
                ...
                public void EntityCreated_DuringUpdate_AcsessComponents()
                {
                    ...
                    scene.Create(new DelegateBehavior(() =>
                    {
                        ...
                        var e = scene.Create(new DelegateBehavior(() => called++));
                ...
                public void EntityCreate_DuringSystem_AcsessComponents()
                {
                    ...
                    foreach (var entity in Enumerable.Range(0, 100).Select(i => scene.Create(i, (float)(i * 2))).ToArray().Where(c => c.Get<int>() % 2 == 0))
                    {
                        entity.Delete();
                    }
        
                    List<GameObject> newEntities = new();
        
                    foreach (GameObject _ in scene.Query<With<int>, With<float>>().EnumerateWithEntities())
                    {
                        var e1 = scene.Create(42, 42f);
                        var e2 = scene.Create(42);
