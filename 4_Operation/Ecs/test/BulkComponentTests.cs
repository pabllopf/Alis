...
namespace Alis.Core.Ecs.Test
{
    ...
    public class BulkComponentTests
    {
        ...
        [Theory, InlineData(10, 1), InlineData(10, 2), InlineData(20, 3), InlineData(30, 4), InlineData(50, 5), InlineData(100, 10)]
        public void BulkComponent_EntityWithComponentCombinations(int entityCount, int componentType)
        {
            ...
            for (int i = 0; i < entityCount; i++)
            {
                ...
                if (componentType >= 5)
                {
                    go.Add(new Damage {Value = i});
                }

                if (componentType >= 6)
                {
                    go.Add(new AnotherComponent {Data = i});
                }

                if (componentType >= 7)
                {
                    go.Add(new AnotherComponent2 {Data = i * 2});
                }

                if (componentType >= 8)
                {
                    go.Add(new Armor {Value = i * 10});
                }
                ...
                if (componentType >= 10)
                {
                    go.Add(new TestComponent {Value = i * 100});
                }
            }
        ...
        [Theory, InlineData(1), InlineData(2), InlineData(5), InlineData(10), InlineData(25), InlineData(50)]
        public void BulkComponent_RemoveComponentsInSequence(int removeCount)
        {
            ...
            if (removeCount >= 6)
            {
                if (go.Has<AnotherComponent>())
                {
                    go.Remove<AnotherComponent>();
                }
            }

            if (removeCount >= 7)
            {
                if (go.Has<AnotherComponent2>())
                {
                    go.Remove<AnotherComponent2>();
                }
            }
        ...
        /// </summary>
        /// <param name="entityCount">The entity count</param>
        [Theory, InlineData(10), InlineData(25), InlineData(50), InlineData(100)]
