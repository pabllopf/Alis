...
// 
//  --------------------------------------------------------------------------
//  File:WorldQueryExtensions.2.cs
// 
...
namespace Alis.Core.Ecs
{
    public static partial class WorldQueryExtensions
    {
...

        {
            ...
            return value;
#else
            ref Query cachedValue = ref CollectionsMarshal.GetValueRefOrAddDefault(scene.QueryCache, QueryHashCache<T1, T2>.Value, out bool exists);
            if (!exists)
            {
                cachedValue = scene.CreateQuery(MemoryHelpers.ReadOnlySpanToImmutableArray([default(T1).Rule, default(T2).Rule]));
            }
