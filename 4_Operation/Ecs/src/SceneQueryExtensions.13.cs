...
namespace Alis.Core.Ecs
{
    ...
    public static partial class SceneQueryExtensions
    {
...

        {
            ...
            if (scene.QueryCache.TryGetValue(QueryHashCache<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13>.Value,
            ...
            value = scene.CreateQuery(MemoryHelpers.ReadOnlySpanToImmutableArray([
                ...
                default(T12).Rule, default(T13).Rule
            ]));
            scene.QueryCache[QueryHashCache<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13>.Value] = value;
            return value;
#else
        ref Query cachedValue =
            ref System.Runtime.InteropServices.CollectionsMarshal.GetValueRefOrAddDefault(scene.QueryCache, QueryHashCache<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13>.Value, out bool exists);
        if (!exists)
        {
            cachedValue =
                scene.CreateQuery(MemoryHelpers.ReadOnlySpanToImmutableArray([default(T1).Rule, default(T2).Rule, default(T3).Rule, default(T4).Rule, default(T5).Rule, default(T6).Rule, default(T7).Rule, default(T8).Rule, default(T9).Rule, default(T10).Rule, default(T11).Rule, default(T12).Rule, default(T13).Rule]));
        }
