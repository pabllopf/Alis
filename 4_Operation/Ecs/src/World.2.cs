...
//it just so happens Archetype and Create both end with "e"
    partial class World
    {
        ...
        {
            var archetypes = Archetype<T1, T2>.CreateNewOrGetExistingArchetypes(this);
            ...
            ref T1 ref1 = ref UnsafeExtensions.UnsafeCast<ComponentStorage<T1>>(components.UnsafeArrayIndex(Archetype<T1, T2>.OfComponent<T1>.Index))[eloc.Index]; ref1 = comp1;
        ref T2 ref2 = ref UnsafeExtensions.UnsafeCast<ComponentStorage<T2>>(components.UnsafeArrayIndex(Archetype<T1, T2>.OfComponent<T2>.Index))[eloc.Index]; ref2 = comp2;
        ...
        public ChunkTuple<T1, T2> CreateMany<T1, T2>(int count)
        {
            ...
            var chunks = new ChunkTuple<T1, T2>()
            {
