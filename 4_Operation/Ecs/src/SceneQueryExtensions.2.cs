...
namespace Alis.Core.Ecs
{
    ...
    public static partial class SceneQueryExtensions
    {
        ...
        if (scene.QueryCache.TryGetValue(QueryHashCache<T1, T2>.Value, out Query value)) return value;
        ...
        return value;
            ...
            ref Query cachedValue =
                ref System.Runtime.InteropServices.CollectionsMarshal.GetValueRefOrAddDefault(scene.QueryCache, QueryHashCache<T1, T2>.Value, out bool exists);
            if (!exists)
            {
                cachedValue =
