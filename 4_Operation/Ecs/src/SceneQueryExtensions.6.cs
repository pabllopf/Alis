...
namespace Alis.Core.Ecs
{
    ...
    public static partial class SceneQueryExtensions
    {
...

        {
            ...
            if (scene.QueryCache.TryGetValue(QueryHashCache<T1, T2, T3, T4, T5, T6>.Value, out Query value)) return value;
        ...
        ref Query cachedValue =
            ref System.Runtime.InteropServices.CollectionsMarshal.GetValueRefOrAddDefault(scene.QueryCache, QueryHashCache<T1, T2, T3, T4, T5, T6>.Value, out bool exists);
        if (!exists)
        {
            cachedValue =
                scene.CreateQuery(MemoryHelpers.ReadOnlySpanToImmutableArray([default(T1).Rule, default(T2).Rule, default(T3).Rule, default(T4).Rule, default(T5).Rule, default(T6).Rule]));
        }
