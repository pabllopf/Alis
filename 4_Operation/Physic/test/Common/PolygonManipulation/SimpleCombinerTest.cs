...
namespace Alis.Core.Physic.Test.Common.PolygonManipulation
{
    ...
    public class SimpleCombinerTest
    {
        ...
        /// <summary>
        ///     Tests ProcessValidPolygon's else branch where poly.Count &lt; 3 after MergeParallelEdges.
        ///     This path is unreachable via the public API (MergeParallelEdges discards its return value),
        ///     so it is tested via reflection.
        /// </summary>
        [Fact]
        public void ProcessValidPolygon_WithLessThanThreeVertices_LogsAndSkips()
        {
            System.Reflection.MethodInfo method = typeof(SimpleCombiner).GetMethod("ProcessValidPolygon",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static);

            Vertices poly = new Vertices(new[] { new Vector2F(0, 0), new Vector2F(1, 0) });
            int polyIndex = 0;
            List<Vertices> polys = new List<Vertices>();

            object[] parameters = new object[] { poly, polyIndex, int.MaxValue, 0.001f, polys };
        ...
        /// <summary>
        ///     Tests RemoveEmptyPolygons when the list contains a polygon with zero vertices.
        ///     This path is the body of the if block (lines 274-276),
        ///     unreachable via the public API, so tested via reflection.
        /// </summary>
        [Fact]
        public void RemoveEmptyPolygons_WithEmptyPolygon_RemovesIt()
        {
            System.Reflection.MethodInfo method = typeof(SimpleCombiner).GetMethod("RemoveEmptyPolygons",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static);
        ...
        /// <summary>
        ///     Tests FindTipIndex where at line 395 both sub-conditions are false
        ///     (tipT != firstT and tipT != secondT), exercising the false branch
        ///     of both the `tipT == firstT` and `tipT == secondT` conditions.
        /// </summary>
        [Fact]
        public void FindTipIndex_NoMatchForBoth_ReturnsTipT()
        {
            System.Reflection.MethodInfo method = typeof(SimpleCombiner).GetMethod("FindTipIndex",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static);

            // tipT=0, firstT=1, secondT=2: first condition false, second condition false
        ...
        /// <summary>
        ///     Tests FindTipIndex where tipT equals firstT at line 395,
        ///     exercising the true branch of the `tipT == firstT` condition.
        ///     (tipT=1 after first if, firstT=1)
        /// </summary>
        [Fact]
        public void FindTipIndex_FirstMatchAtLine395_ReturnsUpdatedTip()
        {
            System.Reflection.MethodInfo method = typeof(SimpleCombiner).GetMethod("FindTipIndex",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static);

            // firstT=0 makes tipT become 1 after first if.
            // At line 395: tipT=1, firstT=1 → match
            int result = (int)method.Invoke(null, new object[] { 1, 0 });

            Assert.Equal(2, result);
        }

        /// <summary>
        ///     Tests FindTipIndex where tipT differs from firstT but matches secondT at line 395,
        ///     exercising the false branch of `tipT == firstT` and the true branch of `tipT == secondT`.
        ///     (tipT=1 after first if, firstT=0, secondT=1)
        /// </summary>
        [Fact]
        public void FindTipIndex_SecondMatchAtLine395_ReturnsUpdatedTip()
        {
            System.Reflection.MethodInfo method = typeof(SimpleCombiner).GetMethod("FindTipIndex",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static);

            // firstT=0 makes tipT become 1 after first if.
            // At line 395: tipT=1, firstT=0 → no match; secondT=1 → match
            int result = (int)method.Invoke(null, new object[] { 0, 1 });
