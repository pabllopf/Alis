...
namespace Alis.Core.Physic.Tools.Cutting.Simple
{
    ...
    public static class CuttingTools
    {
        ...
        public static void SplitShape(Fixture fixture, Vector2F entryPoint, Vector2F exitPoint, out Vertices first,
            out Vertices second)
        {
            ...
            foreach (Vector2F vertex in shape.VerticesPrivate)
            {
            ...
            Vertices vertices = new Vertices(shape.VerticesPrivate);
            ...
            for (int n = 0; n < 2; n++)
            {
                ...
                offset = Vector2F.Normalize(offset);
        ...
        public static bool Cut(World world, Vector2F start, Vector2F end)
        {
            ...
            for (int i = 0; i < fixtures.Count; i++)
            {
                ...
                if (fixtures[i].Body.BodyType != BodyType.Static)
                {
                    ...
                    {
                        Body firstFixture = BodyFactory.CreatePolygon(world, first, fixtures[i].Shape.DensityPrivate,
                    ...
                    {
                        Body secondFixture = BodyFactory.CreatePolygon(world, second, fixtures[i].Shape.DensityPrivate,
                            fixtures[i].Body.Position);
