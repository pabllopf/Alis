...
namespace Alis.Core.Physic.Collision.Shapes
{
	...
	public class PolygonShape : Shape
	{
		...
		public override void ComputeMass(out MassData massData, float denstity)
		{
			...
			// Center of mass
			Debug.Assert(area > Common.Settings.FLT_EPSILON);
			center *= 1.0f / area;
		...
		public PolygonShape()
		{
			...
			/*Debug.Assert(def.Type == ShapeType.PolygonShape);
			_type = ShapeType.PolygonShape;
			PolygonDef poly = (PolygonDef)def;

			// Get the vertices transformed into the body frame.
			_vertexCount = poly.VertexCount;
			Debug.Assert(3 <= _vertexCount && _vertexCount <= Settings.MaxPolygonVertices);
			...
			for (int i = 0; i < _vertexCount; ++i)
			{
				...
				Vec2 edge = _vertices[i2] - _vertices[i1];
				Debug.Assert(edge.LengthSquared() > Common.Settings.FLT_EPSILON * Common.Settings.FLT_EPSILON);
				_normals[i] = Vec2.Cross(edge, 1.0f);
				_normals[i].Normalize();
			}
			...
			for (int i = 0; i < _vertexCount; ++i)
			{
				for (int j = 0; j < _vertexCount; ++j)
				{
					...
					Debug.Assert(s < -Settings.LinearSlop);
				}
			}
			...
			for (int i = 1; i < _vertexCount; ++i)
			{
				...
				Debug.Assert(angle > Settings.AngularSlop);
			}
#endif
			...
			for (int i = 0; i < _vertexCount; ++i)
			{
				...
				Debug.Assert(d.X >= 0.0f);
				Debug.Assert(d.Y >= 0.0f);
				Mat22 A = new Mat22();
		...
		public static void ComputeOBB(out OBB obb, Vec2[] vs, int count)
		{
			...
			Debug.Assert(count <= Settings.MaxPolygonVertices);
			...
			for (int i = 1; i <= count; ++i)
			{
				...
				Debug.Assert(length > Common.Settings.FLT_EPSILON);
			...
			Debug.Assert(minArea < Common.Settings.FLT_MAX);
		}*/
	}
}