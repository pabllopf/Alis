...
using Alis.Core.Physic.Extensions.Controllers.Wind.Curve;
...
namespace Alis.Core.Systems.Physics2D.Config.Extensions.Controllers.Wind.Curve
{
    ...
    public class Curve
    {
        ...
        private float HandlePositionBeforeCurve(float position, CurveKey first, CurveKey last)
        {
            switch (PreLoop)
            {
                ...
                case CurveLoopType.Oscillate:
                    ...
                    return HandleLooping(virtualPos, first, last, cycle);
            }
        ...
        /// <param name="last">The last</param>
        /// <returns>The float</returns>
        private float HandlePositionAfterCurve(float position, CurveKey first, CurveKey last)
        {
            switch (PostLoop)
            {
                case CurveLoopType.Constant:
        ...
        /// <returns>The float</returns>
        private float HandleLooping(float virtualPos, CurveKey first, CurveKey last, int cycle)
        {
            if (PreLoop == CurveLoopType.Oscillate)
            {
                if (cycle % 2 == 0)
                {
                    virtualPos = virtualPos - cycle * (last.Position - first.Position);
                }
                else
                {
                    virtualPos = last.Position - virtualPos + first.Position + cycle * (last.Position - first.Position);
                }
            }
