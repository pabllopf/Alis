				...
				unsafe
				{
					fixed (ContactConstraintPoint* ccPointsPtr = cc.Points)
					{
						for (int j = 0; j < cc.PointCount; ++j)
						{
							...
							Debug.Assert(kNormal > Common.Settings.FLT_EPSILON);
							...
							Debug.Assert(kEqualized > Common.Settings.FLT_EPSILON);
							...
							Debug.Assert(kTangent > Common.Settings.FLT_EPSILON);
						...
						else
						{
							...
							for (; ; )
							{
								...
								if (x.X >= 0.0f && x.Y >= 0.0f)
								{
									...
									Debug.Assert(Common.Math.Abs(vn1 - cp1.VelocityBias) < k_errorTol);
									Debug.Assert(Common.Math.Abs(vn2 - cp2.VelocityBias) < k_errorTol);
#endif
									break;
								}
