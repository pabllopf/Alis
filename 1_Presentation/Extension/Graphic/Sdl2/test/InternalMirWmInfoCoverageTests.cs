...
// 
//  --------------------------------------------------------------------------
//  File:InternalMirWmInfoCoverageTests.cs
// 
...
{
    /// <summary>
    ///     The internal mir wm info coverage tests class
    /// </summary>
    public class InternalMirWmInfoCoverageTests
    {
        /// <summary>
        ...
        {
            InternalMirWmInfo info = default(InternalMirWmInfo);
        ...
        public void InternalMirWmInfo_SetProperties_StoresValuesCorrectly()
        {
            ...
            {
                Connection = new IntPtr(1),
                Surface = new IntPtr(2)
            };

            Assert.Equal(new IntPtr(1), info.Connection);
        ...
        {
            InternalMirWmInfo original = new InternalMirWmInfo { Connection = new IntPtr(100) };
            InternalMirWmInfo copy = original;

            copy.Connection = new IntPtr(200);

            Assert.Equal(new IntPtr(100), original.Connection);
            Assert.Equal(new IntPtr(200), copy.Connection);
        }
    }
}