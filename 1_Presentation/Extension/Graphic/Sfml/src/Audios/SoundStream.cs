...
namespace Alis.Extension.Graphic.Sfml.Audios
{
    ...
    public abstract class SoundStream : ObjectBase
    {
        ...
        public abstract bool OnGetData(out short[] samples);
        ...
        private bool GetData(ref Chunk dataChunk, IntPtr userData)
        {
            ...
            {
                GCHandle handle = GCHandle.Alloc(myTempBuffer, GCHandleType.Pinned);
                try
                {
                    dataChunk.samples = handle.AddrOfPinnedObject();
        ...
        /// <summary>
        ///     Sfs the sound stream is relative to listener using the specified sound stream
        /// </summary>
        /// <param name="soundStream">The sound stream</param>
        /// <returns>The bool</returns>
        [DllImport(Csfml.Audio, CallingConvention = CallingConvention.Cdecl), SuppressUnmanagedCodeSecurity, ExcludeFromCodeCoverage]
