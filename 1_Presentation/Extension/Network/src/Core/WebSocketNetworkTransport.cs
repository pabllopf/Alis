...
namespace Alis.Extension.Network.Core
{
    ...
    public class WebSocketNetworkTransport : INetworkTransport
    {
        ...
        /// </summary>
        public async Task BroadcastAsync(NetworkMessageEnvelope message, string exceptClientId = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            string json = _serializer.SerializeEnvelope(message);
            byte[] buffer = Encoding.UTF8.GetBytes(json);
        ...
        /// </summary>
        public async Task<(string ClientId, NetworkMessageEnvelope Message)> ReceiveAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
