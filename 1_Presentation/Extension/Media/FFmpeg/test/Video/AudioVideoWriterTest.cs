...
namespace Alis.Extension.Media.FFmpeg.Test.Video
{
    ...
    public class AudioVideoWriterTest
    {
        ...
        #region WriteFrame Audio Happy Path (via Reflection Setup)

        /// <summary>
        ///     Tests that <see cref="AudioVideoWriter.WriteFrame(AudioFrame)" /> writes
        ///     frame data to <see cref="AudioVideoWriter.InputDataStreamAudio" /> when the
        ///     writer is opened. Uses reflection to set preconditions that would normally
        ///     be set by <see cref="AudioVideoWriter.OpenWrite" /> (which requires ffmpeg).
        /// </summary>
        [RequireFfmpegFact]
        public void WriteFrame_Audio_WhenOpened_WritesRawDataToInputDataStreamAudio()
        {
            ...
            try
            {
                ...
                FieldInfo inputAudioField = typeof(AudioVideoWriter).GetField("<InputDataStreamAudio>k__BackingField",
                    BindingFlags.NonPublic | BindingFlags.Instance);
                using MemoryStream dataStream = new();
                inputAudioField.SetValue(writer, dataStream);
        ...
        #region CloseWrite Happy Path (via Reflection Setup)

        /// <summary>
        ///     Tests that <see cref="AudioVideoWriter.CloseWrite" /> disposes streams and
        ///     resets <see cref="AudioVideoWriter.OpenedForWriting" /> when the writer is
        ///     opened. Uses reflection to set preconditions that would normally be set
        ///     by <see cref="AudioVideoWriter.OpenWrite" /> (which requires ffmpeg).
        /// </summary>
        [RequireFfmpegFact]
        public void CloseWrite_WhenOpened_ResetsOpenedForWritingAndDisposesStreams()
        {
            string path = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString() + ".mp4");

            try
            {
                using AudioVideoWriter writer = new(path, 640, 480, 30.0, 2, 44100, 16, null, null);
        ...
        #region Dispose With OpenedForWriting (via Reflection Setup)

        /// <summary>
        ///     Tests that <see cref="AudioVideoWriter.Dispose" /> correctly calls
        ///     <see cref="AudioVideoWriter.CloseWrite" /> and resets
        ///     <see cref="AudioVideoWriter.OpenedForWriting" /> when the writer is opened.
        ///     Uses reflection to set preconditions that would normally be set
        ///     by <see cref="AudioVideoWriter.OpenWrite" /> (which requires ffmpeg).
        /// </summary>
        [RequireFfmpegFact]
        public void Dispose_WhenOpenedForWriting_ResetsOpenedForWriting()
        {
            string path = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString() + ".mp4");

            try
            {
                AudioVideoWriter writer = new(path, 640, 480, 30.0, 2, 44100, 16, null, null);
                ...
                bool openedAfter = (bool)openedField.GetValue(writer);
                Assert.False(openedAfter);
            }
            finally
            {
                if (File.Exists(path))
